...
rvv_bench_main(void)
{
	...
	size_t x;
	randState.x ^= target_cycles()*7;
	randState.y += target_cycles() ^ ((uintptr_t)&x + 666*(uintptr_t)mem);
...
#define TIME \
	for (ux beg = target_cycles(), _once = 1; _once; \
	       rv_fencei(), \
	       _cycles += target_cycles() - beg, _once = 0)
