...
int decrypt(unsigned char *e, const unsigned char *sk, const unsigned char *s)
{
	...
	static vec inv[ 64 ][ GFBITS ];
	static vec scaled[ 64 ][ GFBITS ];
	static vec eval[ 64 ][ GFBITS ];

	static vec error[ 64 ];

	static vec s_priv[ 2 ][ GFBITS ];
	static vec s_priv_cmp[ 2 ][ GFBITS ];
	static vec locator[ GFBITS ];
	...
	#ifdef RISCV
	start_count(&decryptBerlekampDecCount);
	#endif
	preprocess(recv, s);
	...
	fft(eval, locator);
	#ifdef RISCV
	end_count(&decryptBerlekampDecCount);
	#endif

	// reencryption and weight check

	#ifdef RISCV
	start_count(&decryptReencryptionCount);
	#endif
	...
	#ifdef RISCV
	end_count(&decryptReencryptionCount);
	#endif

	//

	#ifdef RISCV
	start_count(&decryptWeigthCheckCount);
	#endif
	...
	#ifdef RISCV
	end_count(&decryptWeigthCheckCount);
	#endif
